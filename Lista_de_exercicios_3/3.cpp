...
int main()
{
    int numero1, numero2, numerofinal, operacao, continuar;
    bool eh_int;
    char input[50];
    // DEFININDO AS VARIAVEIS
    
    printf("Bem vindo(a) a calculadora interativa de 2 numeros!\nEscolha seus numeros");
    // ESSE DO WHILE LOOP VAI FORÇAR O USUARIO A DIGITAR UM NUMERO
    do { // continuar enquanto o usuario nao quiser sair
        do {
        ...
        do { // mesma logica dos outros do while loops para forcar a responder algo valido
            eh_int = true;
            printf("\nDeseja continuar?\n1. Sim\n2. Nao\n");
            scanf("%i", &operacao); 
            ...
            if (operacao == 1 || operacao == 2) {
                if (operacao == 1) {
                    continuar = 1; // se quiser continuar, continuar sera 1, ja que essa eh a condicao de do while loop para reiterar
                }
                else if (operacao == 2) {
                    printf("\nFechando calculadora...\n");
                    continuar = 0; // se continuar for diferente de 1 ele nao reitera
                    return (3);
                }
            }
            else {
                printf("Nao entendido! Tente novamente.");
                eh_int = false;
            }
        } while (eh_int == false); 
    
    } while (continuar == 1); // loop principal para nao fechar
